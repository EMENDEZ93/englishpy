...
def home(request, template_name='practise/review/home.html'):
    data={}
    data['verb'] = Present.objects.filter(~Q(verb__in=LearnedWord.objects.filter(topic__routine__user=request.user)
...
def next_verb(request):
    ...
    verb = Present.objects.filter(~Q(verb__in=LearnedWord.objects.filter(topic__routine__user=request.user)
                                     .values_list('word', flat=True))).first()
...
def register_learned_word(request):
    data = {}
    LearnedWord.objects.create(word=request.POST['learned_word'],
